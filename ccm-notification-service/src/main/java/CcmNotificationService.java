...
public class CcmNotificationService extends RouteBuilder {
  ...
  private void processChargeAssessmentPriorityEvents() {
    // use method name as route id
    String routeId = new Object() {}.getClass().getEnclosingMethod().getName();

    from("kafka:{{kafka.topic.chargeassessments-priority.name}}?groupId=ccm-notification-service&maxPollRecords=3&maxPollIntervalMs=4800000")
    .routeId(routeId)
    .log(LoggingLevel.INFO,"Event from Kafka {{kafka.topic.chargeassessments-priority.name}} topic (offset=${headers[kafka.OFFSET]}): ${body}\n" +
      "    on the topic ${headers[kafka.TOPIC]}\n" +
      "    on the partition ${headers[kafka.PARTITION]}\n" +
      "    with the offset ${headers[kafka.OFFSET]}\n" +
      "    with the key ${headers[kafka.KEY]}")
  ...
  private void generateStaticReportEvent() {
    ...
    .choice() // -> JADE-2896-MANU events get a priority topic
      .when(simple("${header[event_message_type]} == 'MANU_FILE'"))
        .setProperty("kafka-topic-reports").simple("{{kafka.topic.reports-priority.name}}")
      .endChoice()
      .otherwise()
        .setProperty("kafka-topic-reports").simple("{{kafka.topic.reports.name}}")
    .end()
    .toD("kafka:${exchangeProperty.kafka-topic-reports}")
    ;
  }
  ...
  private void processCourtCasePriorityEvents() {
    // use method name as route id
    String routeId = new Object() {}.getClass().getEnclosingMethod().getName();

    from("kafka:{{kafka.topic.courtcases-priority.name}}?groupId=ccm-notification-service&maxPollRecords=3&maxPollIntervalMs=4800000")
    .routeId(routeId)
    .log(LoggingLevel.INFO,"Event from Kafka {{kafka.topic.courtcases-priority.name}} topic (offset=${headers[kafka.OFFSET]}): ${body}\n" +
      "    on the topic ${headers[kafka.TOPIC]}\n" +
      "    on the partition ${headers[kafka.PARTITION]}\n" +
      "    with the offset ${headers[kafka.OFFSET]}\n" +
      "    with the key ${headers[kafka.KEY]}")
  ...
  private void processChargeAssessmentChanged() throws HttpOperationFailedException {
    ...
    .choice()
      .when(simple("${exchangeProperty.autoCreateFlag} == 'true' || ${exchangeProperty.caseFound} != '' || ${exchangeProperty.createOverrideFlag} == 'true'"))
        ...
        .toD("kafka:${exchangeProperty.kafka-topic-chargeassessments}") // only push on topic, if auto creation is true
        .setProperty("kpi_status", simple(EventKPI.STATUS.EVENT_CREATED.name()))
        .setProperty("kpi_event_topic_name", simple("${exchangeProperty.kafka-topic-chargeassessments}"))
        .setProperty("kpi_event_topic_recordmetadata", simple("${headers[org.apache.kafka.clients.producer.RecordMetadata]}"))
        .setProperty("kpi_component_route_name", simple(routeId))
        .to("direct:preprocessAndPublishEventCreatedKPI")
        // KPI: restore previous values
  ...
  private void processParticipantMerge() throws HttpOperationFailedException {
    ...
    .choice()
      ...
      .doTry()
        .setProperty("fromMergedKeys",jsonpath("$.fields[?(@.name == 'MergedParticipantKeys')].value"))
      .endDoTry()
      ...
      .end()
      .unmarshal().json()
      .setProperty("fromPersonId").simple("${body[id]}")
      .endChoice()
    .end()
    ...
    .choice()
      ...
      .log(LoggingLevel.DEBUG,"Lookup response = '${body}'")
      .doTry()
        .setProperty("toMergedKeys",jsonpath("$.fields[?(@.name == 'MergedParticipantKeys')].value"))
      .endDoTry()
      .doCatch(Exception.class)
        .log(LoggingLevel.ERROR,"General Exception thrown.")
        .log(LoggingLevel.ERROR,"${exception}")
      .end()
      .unmarshal().json()
      .setProperty("toPersonId").simple("${body[id]}")
      .endChoice()
    .end()
    ...
    .choice()
        .when(simple("${exchangeProperty.fromPersonId} != '' && ${exchangeProperty.toPersonId} != ''"))
          .setHeader("fromPersonId", simple("${exchangeProperty.fromPersonId}"))
          .setHeader("toPersonId", simple("${exchangeProperty.toPersonId}"))
          .setHeader("fromPartid", simple("${exchangeProperty.fromPartId}"))
          .setHeader("toPartid", simple("${exchangeProperty.toPartId}"))
          .setHeader("fromMergedPartKeys", simple("${exchangeProperty.fromMergedKeys}"))
          .setHeader("toMergedPartKeys", simple("${exchangeProperty.toMergedKeys}"))
      ...
      .when(simple("${exchangeProperty.accused_person} != '0'"))
        ...
        .choice()
          .when(simple("${exchangeProperty.accused_person} != '0'"))
            ...
            .choice()
              .when(simple("${exchangeProperty.justinCourtCaseStatus} == 'Return' || ${exchangeProperty.justinCourtCaseStatus} == 'Close' || ${exchangeProperty.justinCourtCaseStatus} == 'Finish'"))
  ...
  private void compileRelatedChargeAssessments() {
    ...
    .choice()
      .when(simple("${body[status]} != 'Inactive' || ${body[key]} == ${body[primaryAgencyFileId]}"))
        .setProperty("dems_agency_files").simple("${body[agencyFileId]}")
        .process(new Processor() {
          @Override
          public void process(Exchange exchange) {
      ...
      .process(new Processor() {
        ...
        public void process(Exchange exchange) {
          ...
          if(bcm.getCourt_file_id() != null && !bcm.getCourt_file_id().isEmpty()) {
            ...
            // and if the related court file isn't the current court file.
            if(!metadata.getCourt_file_id().equalsIgnoreCase(bcm.getCourt_file_id())) {
              relatedCf.add(bcm);
            }
          }
  ...
  private void processCourtCaseChanged() {
    ...
    .choice()
      ...
      .endChoice()
      .otherwise()
        .setProperty("kafka-topic-courtcases").simple("{{kafka.topic.courtcases.name}}")
      .endChoice()
    .end()
    ...
    .process(new Processor() {
      ...
      public void process(Exchange exchange) throws Exception {
        ...
        exchange.getMessage().setBody(cadr, ChargeAssessmentDataRef.class);
        if(cadr != null) {
    ...
    .setProperty("primary_case", simple("${body}"))

    .choice()
      .when(simple("${exchangeProperty.primary_rcc_id} != ''"))
        .setProperty("rcc_id", simple("${exchangeProperty.primary_rcc_id}"))
        .log(LoggingLevel.DEBUG, "${exchangeProperty.rcc_id}")
    ...
    //JADE-2671 - look-up primary rcc for update.
    .choice() // If this is an inactive case, look for the primary, if it exists.  That one should have all agency files listed.
      .when(simple("${exchangeProperty.primary_rcc_id} != '' && ${body[status]} == 'Inactive' && ${exchangeProperty.primary_rcc_id} != ${header.event_key}"))
      ...
      .endChoice()
    .end()
    .log(LoggingLevel.DEBUG, "CaseId: ${exchangeProperty.caseId} CaseStatus:${exchangeProperty.caseStatus}")
    .choice()
      .when(simple("${exchangeProperty.primary_rcc_id} != ''"))
        .setProperty("caseId").simple("${body[id]}")
        .setProperty("caseStatus").simple("${body[status]}")
      .endChoice()
      .otherwise()
        .setProperty("caseId").simple("")
        .setProperty("caseStatus").simple("")
      .endChoice()
    .end()
    ...
    .choice()
      ...
      .otherwise()
        // go through other rccs and check if they exist in dems and is active, if they do, need to do a merge.
        .log(LoggingLevel.WARN, "Not an active primary case or no case found")
    ...
    .choice()
      .when(simple(" ${exchangeProperty.createCase} == 'true' || ${exchangeProperty.createOverrideFlag} == 'true'"))
        .doTry()
          ...
          .marshal().json(JsonLibrary.Jackson, CourtCaseEvent.class)
          .log(LoggingLevel.DEBUG,"Generate converted business event: ${body}")
          .toD("kafka:${exchangeProperty.kafka-topic-courtcases}")

          .setProperty("derived_event_recordmetadata", simple("${headers[org.apache.kafka.clients.producer.RecordMetadata]}"))
          .setProperty("derived_event_topic", simple("${exchangeProperty.kafka-topic-courtcases}"))
    ...
    .log(LoggingLevel.DEBUG, "key value: ${header.event_key}")
    // wireTap makes an call and immediate return without waiting for the process to complete
    // the direct call will wait for a certain time before creating the Report End event.
      ...
      .split(body())
        ...
        .process(new Processor() {
          ...
          public void process(Exchange exchange) throws Exception {
            ...
            if (fileCloseData != null && fileCloseData.getRms_event_type() != null) {
      ...
      .process(new Processor() {
        ...
        public void process(Exchange exchange) throws Exception {
          ...
          if (VerifyAllFileResultsOnlyFor(RMS_PROCESSING_STATUS_MAPPINGS.ACTIVE, closeFileResults) ) {
            ...
            && (closeFileResults.get(DemsListItemFieldData.RMS_PROCESSING_STATUS_MAPPINGS.ACTIVE) <= 0)
            && (closeFileResults.get(DemsListItemFieldData.RMS_PROCESSING_STATUS_MAPPINGS.PEND) <= 0)
            && (closeFileResults.get(DemsListItemFieldData.RMS_PROCESSING_STATUS_MAPPINGS.SEMA) <= 0)
            && (closeFileResults.get(DemsListItemFieldData.RMS_PROCESSING_STATUS_MAPPINGS.RETN) <= 0) ) {
            ...
            else if (closeFileResults.get(DemsListItemFieldData.RMS_PROCESSING_STATUS_MAPPINGS.ACTIVE) == 0 && closeFileResults.get(DemsListItemFieldData.RMS_PROCESSING_STATUS_MAPPINGS.RETN) == 0){
              //Has any PENDING
            ...
            else if (closeFileResults.get(DemsListItemFieldData.RMS_PROCESSING_STATUS_MAPPINGS.RETN).intValue() > 0) {
