...
public class CcmSplunkAdapterTest extends CamelQuarkusTestSupport {

	@Inject
	CamelContext context;

	
	@Produce("direct:publishEventKPIToSplunk")
	...
	public void testPublishEventKPI() throws Exception {
		 ...
		 */
		AdviceWith.adviceWith(context, "publishEventKPIToSplunk", a -> {

			a.weaveById("kafka-send-to-event-splunk").replace().to(mockKafka);
		});
		/**
		 * @author mcostell
		 * 
		...
		EventKPI kpi = new EventKPI(stubbedBaseEvent, "EVENT_CREATED");
		...
		context.createFluentProducerTemplate().withBody(io.vertx.core.json.Json.encode(kpi))
		.to("direct:publishEventKPIToSplunk").send();
		...
		/** Not sure what result to expect here
