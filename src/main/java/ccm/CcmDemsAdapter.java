         ...
         .choice()
            .when().simple("${exchangeProperty.total} > 0")
                  .split()
                    ...
                    //.setProperty("identifierValueLength", simple("${body[identifierValue].length}"))
                    //.setHeader("identifierValue", jsonpath("$.items[0].identifierValue"))
                    //.setProperty("identifierValueLength", simple("${header.identifierValue.length()}"))
                    .setProperty("identifierValueLength", simple("${header.identifierValue.length()}"))
                    .log(LoggingLevel.INFO,"Length : ${exchangeProperty.identifierValueLength}")
                        .choice()
                          .when().simple("${exchangeProperty.total} > 0 && ${exchangeProperty.identifierValueLength} == 6")
                            ...
                            .log(LoggingLevel.INFO,"id : ${exchangeProperty.idValue}")
                            
                            /*.setBody(simple("{\"id\":\"${exchangeProperty.idValue}\",\"identifierType\":\"${exchangeProperty.newidentifierValue}\"}"))
                            .removeHeader("CamelHttpUri")
                            .removeHeader("CamelHttpBaseUri")
                            .removeHeaders("CamelHttp*")
                            .setHeader(Exchange.HTTP_METHOD, simple("PUT"))
