...
Helm/
├── base/
│   └── ccm-lookup-service.yml             # Base configuration
├── dev/
│   └── ccm-lookup-service.yml             # Dev overrides (2 replicas + HPA)
├── test/
│   └── ccm-lookup-service.yml             # Test overrides (2 replicas + HPA)
├── prod/
│   └── ccm-lookup-service.yml             # Prod overrides (1 replica)
└── templates/
    ├── deployment.yml
    ├── service.yml
    ├── config-map.yml
...
```bash
# Scale to 5 replicas in dev (override the default 2)
helm upgrade --install ccm-lookup-service . \
  -f base/ccm-lookup-service.yml \
  -f dev/ccm-lookup-service.yml \
  --set replicas=5 \
